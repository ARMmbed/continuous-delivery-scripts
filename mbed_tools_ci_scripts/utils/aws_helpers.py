...
import mimetypes
from mbed_tools_ci_scripts.utils.configuration import ConfigurationVariable, \
    configuration
...
def _get_aws_config() -> Tuple[str, dict]:
    ...
    s3_config = {
        ...
        "aws_secret_access_key": configuration.get_value(
...
def _upload_directory_file_contents(bucket_name: str,
                                    files: List[str], bucket_dir: str,
                                    root: str) -> None:
    for name in files:
        upload_file(Path(root).joinpath(name), bucket_dir, bucket_name)


def _upload_directory_directories(bucket_name: str,
                                  dirs: List[str], bucket_dir: str,
                                  root: str) -> None:
    for name in dirs:
        upload_directory(Path(root).joinpath(name), bucket_dir, bucket_name)


def _determine_destination(bucket_dir: str, real_dir_path: Path,
                           current_folder: Path) -> str:
    if current_folder != real_dir_path:
        bucket_dest = str(Path(bucket_dir).joinpath(
            current_folder.relative_to(real_dir_path)))
    else:
...
def upload_directory(dir: Path, bucket_dir: str, bucket_name: str) -> None:
    ...
    for root, dirs, files in os.walk(str(real_dir_path), followlinks=True,
                                     onerror=onerror):
        new_bucket_dir = _determine_destination(bucket_dir, real_dir_path,
                                                Path(root))
        _upload_directory_directories(bucket_name, dirs, new_bucket_dir,
                                      root)
