...
class TestGenerateDocs(TestCase):
    ...
    def test_generate_docs(self, TemporaryDirectory, check_call,
                           _clear_previous_docs):
        ...
        with Patcher():
            ...
            check_call.assert_called_once_with(
                _generate_pdoc_command_list(temp_dir, fake_module)
            )
    ...
    @mock.patch("mbed_tools_ci_scripts.generate_docs.check_call")
    @mock.patch("mbed_tools_ci_scripts.generate_docs.TemporaryDirectory")
    def test_generate_docs_errors(self, TemporaryDirectory, check_call,
                                  log_exception,
                                  _clear_previous_docs):
    ...
    def test_update_docs(self, _call_pdoc,
                         TemporaryDictionary):
        with Patcher() as patcher:
            temp_dir = pathlib.Path("temp")
